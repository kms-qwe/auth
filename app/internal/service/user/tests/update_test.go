...
func TestUpdate(t *testing.T) {
	...
	var (
		...
		reqCorrect = &model.UserInfoUpdate{
			ID:    id,
			Name:  &name,
			Email: &email,
			Role:  role,
		...
		reqEmpty = &model.UserInfoUpdate{}

		logCorrect = fmt.Sprintf("user updated:%#v", reqCorrect)
		logEmpty   = fmt.Sprintf("user updated:%#v", reqEmpty)
	)
	...
	tests := []struct {
	...
	}{
		{
			...
			userRepositoryMock: func(mc *minimock.Controller) repository.UserRepository {
				...
				mock.UpdateMock.Expect(ctx, reqCorrect).Return(nil)
				return mock
			},
...

		{
			...
			userRepositoryMock: func(mc *minimock.Controller) repository.UserRepository {
				mock := repositoryMock.NewUserRepositoryMock(mc)
				mock.UpdateMock.Expect(ctx, reqEmpty).Return(nil)
				return mock
			},
			logRepositoryMock: func(mc *minimock.Controller) repository.LogRepository {
				mock := repositoryMock.NewLogRepositoryMock(mc)
...

		{
			...
			userRepositoryMock: func(mc *minimock.Controller) repository.UserRepository {
				mock := repositoryMock.NewUserRepositoryMock(mc)
				mock.UpdateMock.Expect(ctx, reqCorrect).Return(userRepoErr)
				return mock
			},
			logRepositoryMock: func(mc *minimock.Controller) repository.LogRepository {
				mock := repositoryMock.NewLogRepositoryMock(mc)
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := userService.Update(tt.args.ctx, tt.args.req)
